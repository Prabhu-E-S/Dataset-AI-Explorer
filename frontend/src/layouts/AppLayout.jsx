...
export default function AppLayout({ children, datasets = [], onDeleteDataset, onSearchChange, searchTerm }) {
    ...
    const handleDatasetDelete = async (id) => {
        if (!window.confirm("Are you sure you want to delete this dataset? This action is irreversible.")) return;
        try {
            ...
            console.error("Failed to delete dataset from sidebar:", err);
            ...
            <header className="sticky top-0 bg-brand-sidebar/85 backdrop-blur-md border-b border-brand-border h-14 flex items-center justify-between px-4 z-40">
                <div className="flex items-center gap-3">
                    ...
                    <button
                        ...
                        className="md:hidden p-1.5 text-brand-muted hover:text-white rounded-lg hover:bg-brand-hover/50 active-scale"
                    >
            ...
            <div className="flex flex-1 relative overflow-hidden">
                    ...
                    {mobileMenuOpen && (
                        ...
                        >
                            <div className="flex items-center justify-between mb-4 pb-2 border-b border-brand-border/40">
                                ...
                                <button
                                    onClick={() => setMobileMenuOpen(false)}
                                    className="p-1 text-brand-muted hover:text-white rounded hover:bg-brand-hover/50 active-scale"
                                >
                                    <X className="text-lg" />
                                </button>
                            </div>
                ...
                >
                    ...
                    <button
                        ...
                        className="absolute top-1/2 -translate-y-1/2 -right-2.5 bg-brand-card hover:bg-brand-hover text-brand-text border border-brand-border rounded-full p-1 shadow-sm z-30 active-scale"
                    >
                        {sidebarCollapsed ? <CaretDoubleRight size={11} /> : <CaretDoubleLeft size={11} />}
                    </button>
                    ...
                    <div className="flex-1 flex flex-col p-3.5 overflow-hidden">
                        {sidebarCollapsed ? (
                            <div className="flex flex-col items-center gap-5 py-2">
                                <button
                                    onClick={() => navigate('/')}
                                    className={`p-2.5 rounded-lg border border-dashed active-scale ${isUploadPage
                                ...
                                <div className="flex-grow flex flex-col gap-2 overflow-y-auto px-0.5 w-full items-center custom-scrollbar">
                                    {datasets.map(d => (
                                        <button
                                            ...
                                            className={`p-2 rounded-lg border active-scale ${location.pathname.includes(d.id)
    ...
    return (
        <div className="flex flex-col h-full overflow-hidden select-none">
            ...
            <button
                ...
                className={`w-full flex items-center justify-center gap-2 py-2 px-3 rounded-lg font-semibold border active-scale transition-all duration-200 ${isUploadPage
            ...
            >
                <CloudArrowUp className="text-base" />
            ...
            <div className="flex-1 overflow-y-auto flex flex-col gap-1.5 pr-1 custom-scrollbar">
                {datasets.length === 0 ? (
                    <div className="flex flex-col items-center justify-center py-8 px-3 text-center border border-dashed border-brand-border rounded-lg bg-brand-bg/20">
                        <Database className="text-brand-muted text-xl mb-1.5" />
                    ...
                    datasets
                        ...
                        .map(d => {
                            ...
                            return (
                                <div
                                    key={d.id}
                                    className={`group relative w-full flex items-center justify-between p-2.5 rounded-lg border cursor-pointer active-scale transition-all duration-150 ${isActive
                                ...
                                >
                                    <div className="flex items-center gap-2.5 overflow-hidden pr-5 text-left">
                                        <Database className={`text-sm flex-shrink-0 ${isActive ? 'text-brand-primary' : 'text-brand-muted group-hover:text-white/80'}`} />
                                    ...
                                    <button
                                        ...
                                        className="absolute right-2.5 opacity-0 group-hover:opacity-100 p-1 text-brand-muted hover:text-red-400 hover:bg-brand-hover rounded transition-all duration-100 active-scale"
